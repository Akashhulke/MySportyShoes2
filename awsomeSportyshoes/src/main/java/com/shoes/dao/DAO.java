...
import com.shoes.model.Purchase;
...
public class DAO {
	...
	public String authenticate(String user, String password) {
		...
		return NPjdbcTemplate.queryForObject("select password from user where name =:user", namedParameters,
				String.class);
	}
	...
	public List<Purchase> searchAllPurchases() {
		return jdbcTemplate.query(
				"select u.name, p.name, p.category, pu.timestamp from purchase pu left join (user u, product p) on (pu.id = u.userId and pu.id = p.id)",
	...
	public List<Purchase> searchPurchasesByDate(String date) {
		return jdbcTemplate.query(
				"select u.name, p.name, p.category, pu.timestamp from purchase pu left join (user u, product p) on (pu.id = u.userId and pu.id = p.id) where Date(pu.timestamp)=?",
	...
	public List<Purchase> searchPurchasesByCategory(String category) {
		return jdbcTemplate.query(
				"select u.name, p.name, p.category, pu.timestamp from purchase pu left join (user u, product p) on (pu.id = u.userId and pu.id = p.id) where p.category=?",
				new PreparedStatementSetter() {
					public void setValues(PreparedStatement preparedStatement) throws SQLException {
